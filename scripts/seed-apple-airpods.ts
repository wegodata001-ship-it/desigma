...
import { seedAppleAirpodsCatalog } from "../src/lib/presets/appleAirpodsPreset";
...
/** Prefer transaction pooler / direct host for seeding (avoid session pool limits). */
...
async function main() {
  await seedAppleAirpodsCatalog(prisma, STORE_ID);
  console.log(`Apple AirPods catalog (Stage 2) seeded for store: ${STORE_ID}`);
}
