import { Prisma, PrismaClient } from "@prisma/client";
import {
  APPLE_AIRPODS,
  APPLE_AUDIO_CATEGORY,
  airpodsDescription,
  airpodsGallery,
  airpodsImage,
  buildAirpodsSpecs,
  type AirpodsSeed,
} from "../catalog/apple-airpods-catalog";

const PRODUCT_SKU_PREFIX = "APPLE-AIRPODS";

function productSku(p: AirpodsSeed): string {
  return `APPLE-${p.key.toUpperCase()}`;
}

function variantSku(p: AirpodsSeed, colorSlug: string): string {
  return `${productSku(p)}-${colorSlug.toUpperCase()}`;
}

function localizedSpecs(p: AirpodsSeed) {
  return {
    he: buildAirpodsSpecs("he", p.name_he, p.spec),
    ar: buildAirpodsSpecs("ar", p.name_ar, p.spec),
    en: buildAirpodsSpecs("en", p.name_en, p.spec),
  };
}

async function upsertCategories(prisma: PrismaClient, storeId: string): Promise<string> {
  const parent = APPLE_AUDIO_CATEGORY.parent;
  const child = APPLE_AUDIO_CATEGORY.airpods;
  const parentId = `${storeId}-cat-${parent.key}`;
  const childId = `${storeId}-cat-${child.key}`;

  await prisma.category.upsert({
    where: { id: parentId },
    create: {
      id: parentId,
      storeId,
      name_he: parent.name_he,
      name_ar: parent.name_ar,
      name_en: parent.name_en,
      imageUrl: parent.imageUrl,
      active: true,
      sortOrder: parent.sortOrder,
    },
    update: {
      name_he: parent.name_he,
      name_ar: parent.name_ar,
      name_en: parent.name_en,
      imageUrl: parent.imageUrl,
      active: true,
      sortOrder: parent.sortOrder,
    },
  });

  await prisma.category.upsert({
    where: { id: childId },
    create: {
      id: childId,
      storeId,
      parentId,
      name_he: child.name_he,
      name_ar: child.name_ar,
      name_en: child.name_en,
      imageUrl: child.imageUrl,
      active: true,
      sortOrder: child.sortOrder,
    },
    update: {
      parentId,
      name_he: child.name_he,
      name_ar: child.name_ar,
      name_en: child.name_en,
      imageUrl: child.imageUrl,
      active: true,
      sortOrder: child.sortOrder,
    },
  });

  return childId;
}

async function upsertVariants(prisma: PrismaClient, productId: string, p: AirpodsSeed) {
  await prisma.productVariantGroup.deleteMany({ where: { productId } });

  const perColorStock = Math.max(1, Math.round(40 / p.colors.length));

  await prisma.productVariantGroup.create({
    data: {
      productId,
      name: "Color",
      sortOrder: 0,
      options: {
        create: p.colors.map((c, i) => ({
          value: c.value,
          priceAdd: new Prisma.Decimal(0),
          stock: perColorStock,
          image: airpodsImage(p.key, c.slug, "main"),
          sku: variantSku(p, c.slug),
          isDefault: i === 0,
          sortOrder: i,
        })),
      },
    },
  });
}

async function upsertImages(prisma: PrismaClient, storeId: string, productId: string, p: AirpodsSeed) {
  await prisma.productImage.deleteMany({ where: { storeId, productId } });

  let sortOrder = 0;
  const data = p.colors.flatMap((c) =>
    airpodsGallery(p.key, c.slug).map((img) => ({
      storeId,
      productId,
      url: img.url,
      alt: `${p.name_en} — ${c.value} (${img.kind})`,
      isMain: sortOrder === 0 ? true : false,
      sortOrder: sortOrder++,
    })),
  );

  await prisma.productImage.createMany({ data });
}

/** Idempotent seed of the REAL Apple AirPods catalog (Stage 2). */
export async function seedAppleAirpodsCatalog(prisma: PrismaClient, storeId: string): Promise<void> {
  const categoryId = await upsertCategories(prisma, storeId);
  const activeSkus: string[] = [];

  for (const p of APPLE_AIRPODS) {
    const sku = productSku(p);
    activeSkus.push(sku);

    const oldPrice = p.salePercent
      ? Math.round((p.basePrice / (1 - p.salePercent / 100)) / 10) * 10
      : null;
    const discountPercent =
      oldPrice && oldPrice > p.basePrice
        ? Math.round(((oldPrice - p.basePrice) / oldPrice) * 100)
        : null;

    const specs = localizedSpecs(p);
    const desc = airpodsDescription(p);
    const title = {
      he: `${p.name_he} — קנו אונליין | אחריות רשמית`,
      ar: `${p.name_ar} — اشترِ أونلاين | ضمان رسمي`,
      en: `${p.name_en} — Buy Online | Official Warranty`,
    };

    const perColorStock = Math.max(1, Math.round(40 / p.colors.length));
    const totalStock = perColorStock * p.colors.length;

    const saved = await prisma.product.upsert({
      where: { storeId_sku: { storeId, sku } },
      create: {
        storeId,
        categoryId,
        title_he: title.he,
        title_ar: title.ar,
        title_en: title.en,
        name_he: p.name_he,
        name_ar: p.name_ar,
        name_en: p.name_en,
        description_he: desc.he,
        description_ar: desc.ar,
        description_en: desc.en,
        specs_he: specs.he as unknown as Prisma.InputJsonValue,
        specs_ar: specs.ar as unknown as Prisma.InputJsonValue,
        specs_en: specs.en as unknown as Prisma.InputJsonValue,
        price: new Prisma.Decimal(p.basePrice),
        oldPrice: oldPrice != null ? new Prisma.Decimal(oldPrice) : null,
        discountPercent,
        stock: totalStock,
        sku,
        active: true,
        featured: p.featured === true,
        tags: p.tags,
      },
      update: {
        categoryId,
        title_he: title.he,
        title_ar: title.ar,
        title_en: title.en,
        name_he: p.name_he,
        name_ar: p.name_ar,
        name_en: p.name_en,
        description_he: desc.he,
        description_ar: desc.ar,
        description_en: desc.en,
        specs_he: specs.he as unknown as Prisma.InputJsonValue,
        specs_ar: specs.ar as unknown as Prisma.InputJsonValue,
        specs_en: specs.en as unknown as Prisma.InputJsonValue,
        price: new Prisma.Decimal(p.basePrice),
        oldPrice: oldPrice != null ? new Prisma.Decimal(oldPrice) : null,
        discountPercent,
        stock: totalStock,
        active: true,
        featured: p.featured === true,
        tags: p.tags,
      },
    });

    await upsertVariants(prisma, saved.id, p);
    await upsertImages(prisma, storeId, saved.id, p);
  }

  // Deactivate any demo AirPods/headphone products from earlier seeds.
  await prisma.product.updateMany({
    where: { storeId, sku: { startsWith: "AUDIO-AIRPODS" } },
    data: { active: false },
  });

  // Deactivate stale real-catalog SKUs not in the current lineup.
  await prisma.product.updateMany({
    where: { storeId, sku: { startsWith: PRODUCT_SKU_PREFIX }, NOT: { sku: { in: activeSkus } } },
    data: { active: false },
  });
}
