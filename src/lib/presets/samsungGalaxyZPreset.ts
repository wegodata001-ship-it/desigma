...
import {
  SAMSUNG_GALAXY_Z,
  SAMSUNG_GALAXY_Z_CATEGORY,
  buildZSpecs,
  zDescription,
  zGallery,
  zImage,
  type ZSeed,
} from "../catalog/samsung-galaxy-z-catalog";

const PRODUCT_SKU_PREFIX = "SAMSUNG-GALAXY-Z";
const PER_COLOR_STOCK = 10;

function productSku(p: ZSeed): string {
  return `SAMSUNG-${p.key.replace(/^samsung-/, "").toUpperCase()}`;
}

function localizedSpecs(p: ZSeed) {
  return {
    he: buildZSpecs("he", p),
    ar: buildZSpecs("ar", p),
    en: buildZSpecs("en", p),
...
async function upsertCategories(prisma: PrismaClient, storeId: string): Promise<string> {
  const parent = SAMSUNG_GALAXY_Z_CATEGORY.parent;
  const child = SAMSUNG_GALAXY_Z_CATEGORY.galaxyZ;
  const parentId = `${storeId}-cat-${parent.key}`;
  const childId = `${storeId}-cat-${child.key}`;

  // Parent "Smartphones" is shared — upsert without clobbering its image.
...
async function upsertVariants(prisma: PrismaClient, productId: string, p: ZSeed) {
    ...
    data: {
      ...
      options: {
        create: p.colors.map((c, i) => ({
          ...
          image: zImage(p.key, c.slug, "front"),
...
async function upsertImages(prisma: PrismaClient, storeId: string, productId: string, p: ZSeed) {
  await prisma.productImage.deleteMany({ where: { storeId, productId } });

  let sortOrder = 0;
  const data = p.colors.flatMap((c) =>
    zGallery(p.key, c.slug).map((img) => ({
      storeId,
      productId,
      url: img.url,
...
/** Idempotent seed of the REAL Samsung Galaxy Z catalog (Master Catalog Part 2). */
export async function seedSamsungGalaxyZCatalog(prisma: PrismaClient, storeId: string): Promise<void> {
  const categoryId = await upsertCategories(prisma, storeId);
  const activeSkus: string[] = [];

  for (const p of SAMSUNG_GALAXY_Z) {
    ...
    const desc = {
      he: zDescription("he", p),
      ar: zDescription("ar", p),
      en: zDescription("en", p),
    };
    const title = {
